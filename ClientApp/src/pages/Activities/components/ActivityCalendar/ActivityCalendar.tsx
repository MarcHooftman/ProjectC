...
function ActivityCalendar({ activities = [] }: Props) {
  ...
  const tileContent = ({ date }: { date: Date }) => {
    ...
    return activitiesOnThisDay.map((activity, index) => {
      return (
        <span
          key={index}
          className="d-flex align-items-center gap-1 tile-activity-line"
        >
          <div className="tile-activity-dot"></div>
          <b key={activity.id} className="tile-activity-title">
            {activity.title}
          </b>
        </span>
      );
  ...
  return (
    <Card className="shadow-lg calendar-card" role="calendar-card">
      <Card.Body>
        <Calendar
