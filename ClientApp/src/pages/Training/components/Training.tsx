...
const Training = () => {
  ...
  filterTraining(Trainings)?.forEach((i) => {
    if (i.tags.length === 0) Category.push({ name: "Geen Tag" });
    if (!Category.some((_) => _.name.includes(i.tags[0]?.name)))
      Category.push(i.tags[0]);
  });
  return (
    <Layout role="training-page">
      <CustomAuthenticatedTemplate>
        ...
        {filterTraining(Trainings).length > 0 ? (
          Category.map((i, index) => {
            return (
              <div key={index} className="ps-3 mb-5">
                ...
                <div className="d-flex pt-3 gap-4 flex-row flex-wrap justify-content-start">
                  {filterTraining(Trainings).map((item, itemIndex) => {
                    if (
                      item.tags[0]?.name.includes(i?.name) ||
                      (item.tags.length == 0 && i?.name == "Geen Tag")
                    ) {
                      return (
                        <TrainingInfoCard
                          key={itemIndex}
          ...
          <h4 className="blue-text opacity-75">
            Er zijn hier nog geen trainingen...
          </h4>
        )}
      </CustomAuthenticatedTemplate>
    </Layout>
