...
const ReportButton = ({ postId, profileId, onClick = () => { }, onSubmit = () => { } }: Props) => {
    ...
    const [inappropriateContentChecked, setInappropriateContentChecked] = useState<boolean>(false);
    const [nonWorkRelatedChecked, setNonWorkRelatedChecked] = useState<boolean>(false);
    ...
    const refreshReported = () => {
        fetch(`https://localhost:7185/api/forumpost/${postId}`)
            ...
            .then((data) => {
                ...
                setReported(post.reports?.some((report) => report.profileID == profileId));
            });
    }
    ...
    const handleOtherCheckedChange = (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            >
                ...
                <Modal.Body>
                    ...
                    <Form onSubmit={handleFormSubmit}>
                        <Form.Check>
                            <Form.Check.Input type="checkbox" checked={spamChecked} onChange={(event) => setSpamChecked(event.target.checked)} />
                            <Form.Check.Label>Het is spam</Form.Check.Label>
                        </Form.Check>
                        <Form.Check>
                            <Form.Check.Input type="checkbox" checked={inappropriateContentChecked} onChange={(event) => setInappropriateContentChecked(event.target.checked)} />
                            <Form.Check.Label>Ongepaste inhoud</Form.Check.Label>
                        </Form.Check>
                        <Form.Check>
                            <Form.Check.Input type="checkbox" checked={nonWorkRelatedChecked} onChange={(event) => setNonWorkRelatedChecked(event.target.checked)} />
                            <Form.Check.Label>Niet werk-gerelateerd</Form.Check.Label>
                        </Form.Check>
                        <Form.Check>
                            <Form.Check.Input type="checkbox" id="other" onChange={(event) => handleOtherCheckedChange(event)} />
                            <Form.Check.Label>Anders</Form.Check.Label>
                            {showOtherInput && (
                                <Form.Control type="text" placeholder="Toelichting" value={otherInputValue} onChange={(event) => setOtherInputValue(event.target.value)} />
                            )}
                        </Form.Check>
