...
const UserDataCard = ({ posts, className = "" }: Props) => {
  ...
  if (Array.isArray(posts)) {
    totalLikes =
      posts?.reduce(
        (currentTotal, post) => currentTotal + post?.likes?.length,
        0
      ) || 0;
  }

  const postsLength = Array.isArray(posts) ? filterOnlyParent(posts).length : 0;
  const commentsPlaced = Array.isArray(posts)
    ? posts.length - filterOnlyParent(posts).length
    : 0;
  const helpfulComments = Array.isArray(posts)
    ? filterHelpfulComment(posts).length
    : 0;

  return (
    <Card
      className={"user-data shadow-lg bg-antes-primary".concat(" ", className)}
      role="user-data-card"
    >
