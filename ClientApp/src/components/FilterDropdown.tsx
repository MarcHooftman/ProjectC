...
const FilterDropdown = ({ page, options = [] }: Props) => {
    ...
    const handleNoFilterClick = (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
        event.preventDefault();
        setShow(false);
        navigate(`/${page}`);
    }
    ...
    return (
        <Dropdown autoClose={true} show={show} onToggle={() => setShow(!show)}>
            ...
            <Dropdown.Menu align="end">
                ...
                <Dropdown.Divider />
                <Dropdown.Item onClick={handleNoFilterClick} className="fs-6 blue-text">No filter</Dropdown.Item>
                {options.length > 0 && <>
                    {options.map(option => <Dropdown.Item href={`/${page}?filter=${option}`} className="fs-6 blue-text">{option}</Dropdown.Item>)}
                </>}
            </Dropdown.Menu>
        </Dropdown>
    )
